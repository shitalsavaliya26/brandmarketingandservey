            ...
            <tbody>
                ...
                @foreach($withdraws as $row)
                    <tr class="text-center">
                        ...
                        <td class="text-left">{{ isset($row->currencyname->symbol)?$row->currencyname->symbol:"$" }} {{number_format((float)$row->amount, 3, '.', '')}}</td>
                        <td>
                        @if($row->status== '1')
                        ...
                        <td>
                            @if($row->status== '2')
                                <span class="ti-check bg-success text-white topup-status change-status" id="approve" data-id="{{$row->id}}">Approve</span>
                                <span class="ti-close bg-danger text-white topup-status change-status" id="refuse" data-id="{{$row->id}}">Refuse</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
